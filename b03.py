...
for i, H, L in [(0.10, 0, 0), (0.4, 0, 1), (0.95, 1, 0), (0.8, 1, 1)]:
    ...
    axes[H, L].scatter(np.array(x)[model_svr.support_], y[model_svr.support_], marker='p', s=120, alpha=0.2,
                       label='支持向量')
...
figure1.savefig('a333.png',dpi=400)

# 选取一组数据点进行预测，输出均值和方差
xx = [x[5], x[214], x[125], x[69], x[320]]
yy=[y[5], y[214], y[125], y[69], y[320]]
xx=np.array(xx)
yy=np.array(yy)
...
print("    线性回归的均值为：%.3f, 方差为：%.3f" % (np.mean(yy-yy_line), np.var(yy-yy_line)))
print("    支持向量回归的均值为：%.3f, 方差为：%.3f" % (np.mean(yy-yy_svr), np.var(yy-yy_svr)))